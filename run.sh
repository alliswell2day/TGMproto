...
echo "$SECRET" | tr ',' '\n' | while read S; do
  echo "[*]   Secret $I: dd$S"
  echo "[*]   tg:// link for secret $I auto configuration: tg://proxy?server=${IP}&port=9000&secret=dd${S}"
  echo "[*]   t.me link for secret $I: https://t.me/proxy?server=${IP}&port=9000&secret=dd${S}"
  I=$(($I+1))
done
...
echo '[+] Starting proxy...'
sleep 1
exec /usr/local/bin/mtproto-proxy -p 2398 -H 9000 -M "$WORKERS" -C 60000 --aes-pwd /etc/telegram/proxy-secret -u root $CONFIG --allow-skip-dh --nat-info "$INTERNAL_IP:$IP" $SECRET_CMD $TAG_CMD
